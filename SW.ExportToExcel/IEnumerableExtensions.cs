...
namespace SW.ExportToExcel
{
    public static class IEnumerableExtensions
    {
        ...
        public static byte[] ExportToExcel<TEntity>(this IEnumerable<TEntity> data)
        {
            var dictionary = typeof(TEntity).GetProperties().ToDictionary(k => k.Name, v => v.Name);
            return ExportToExcel(data, dictionary);
        }

        public static byte[] ExportToExcel<TEntity>(this IEnumerable<TEntity> data, IEnumerable<string> columns)
        {
            var dictionary = columns.ToDictionary(k => k, v => v);
            return ExportToExcel(data, dictionary);
        }

        public static byte[] ExportToExcel<TEntity>(this IEnumerable<TEntity> data, IDictionary<string, string> columns)
        {
            var tempFile = Path.GetTempFileName();

            try
            {
                using (var doc = SpreadsheetDocument.Create(tempFile, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    WriteXmlToPart(sharedStringTablePart, xmlStringTable);
                    ...
                    WriteXmlToPart(workbookPart, workbookElement);
                    ...
                    WriteXmlToPart(worksheetPart, worksheetElement);

                    var styleSheetElement = XElement.Parse(styleSheet);

                    WriteXmlToPart(stylesheetPart, styleSheetElement);
                }
